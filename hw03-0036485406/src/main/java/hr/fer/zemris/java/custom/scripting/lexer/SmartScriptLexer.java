...
public class SmartScriptLexer {
	...
	private void tagMode() {
		...
		if (startsWith(QUOTES)) {
			...
			if (value.length() == 0) {
				throw new LexerException("Između para '\"' mora biti barem jedan znak.");
			}
			currentIndex += QUOTES.length();
		...
		if (startsWith(AT_SIGN)) {
			...
			String value = extractVariableName();
			if (value.length() == 0) {
				throw new LexerException("Poslije '@' mora ići naziv funkcije.");
			}
	...
	private Number extractNumber() {
		...
		try {
			...
			throw new LexerException("Broj '" + value + "'nije valjan.");
		}
	}
