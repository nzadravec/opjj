...
public class HexdumpShellCommand extends AbstractShellCommand {

	{
		 ...
		 * hex-output. For all other characters a '.' is printed instead (i.e. replace
		 * all bytes whose value is less than 32 or greater than 127 with ' . ').
		 */
		commandDescription.add("Hexdump command expects a single argument: file name, and produces");
		commandDescription.add("hex-output. For all other characters a '.' is printed instead (i.e. replace");
		commandDescription.add("all bytes whose value is less than 32 or greater than 127 with ' . ').");
	}
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		if (!Files.isRegularFile(file)) {
			env.writeln("Command hexdump takes file as argument.");
			return ShellStatus.CONTINUE;
		}
